'use client';
import XMLResultsMetalumen from '../Actualizer_Excel_Metalumen/components/XML_Results_Metalumen.jsx';
import ImportCETXml from '../Actualizer_Excel_Metalumen/components/IncertDataExcel/incertXML_excel';
...
import AIReporting from './components/comparePDF/presentation_Metalumen.jsx'
...
import CETComparator from '../Actualizer_Excel_Metalumen/components/CET_Comparator.jsx';
...
export default function MenuInicial() {
  ...
  useEffect(() => {
    const checkDelegation = async () => {
      try {
        ...
        if (!user) {
          window.location.href = '/Metalumen';
          return;
        }
        
        const apiURL = process.env.NEXT_PUBLIC_API_URL || 'https://servex-ai-back.onrender.com';
        const res = await fetch(`${apiURL}/api/v1/module_delegation/Metalumen`);
  ...
  const renderContent = () => {
    ...
    switch (active) {
      ...
      case 'xml_results': return <XMLResultsMetalumen />;
              ...
              <TeamsAgentChat 
                ...
                renderTool={(toolId) => {
                  switch (toolId) {
                    ...
                    case 'incert_wbs_csv_base': return <IncertDelete step="csv_base" />;
                    case 'incert_wbs_csv_new': return <IncertDelete step="csv_new" />;
                    case 'import_cet_xml': return <div className="w-full h-full overflow-y-auto"><ImportCETXml moduleName="Metalumen" /></div>;
                    case 'delete_data': return <div className="w-full max-w-sm mx-auto mt-4"><DeleteData /></div>;
                    case 'report': return <Report />;
                    case 'graphics': return <ViewportGraphics />;
                    case 'AI_reporter': return <Responce_ai />;
                    case 'exportCETcsv': return <XMLResultsMetalumen />;
