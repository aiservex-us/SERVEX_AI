...
export default function DataViewer() {
  ...
  const fetchLatestData = async () => {
    ...
    try {
      ...
      console.error('Error fetching MityLite data:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDownloadCSV = () => {
    ...
    link.href = url;
    const filename = `${data?.company_name || 'MityLite'}_${activeTab === 'csv_raw' ? 'Sanitized_Manual' : 'Sanitized_PDF'}_${new Date().toISOString().slice(0,10)}.csv`;
  ...
  if (loading) return (
    <div className="flex items-center justify-center min-h-[90vh] bg-white text-xs font-semibold text-slate-500 font-sans">
      <div className="flex items-center gap-2">
        ...
        Retrieving master data matrix for MityLite...
      </div>
    </div>
  );
