  ...
  const saveSingleStep = async (type: 'xml' | 'csv_base' | 'csv_new', rawContent: string) => {
    ...
    try {
      ...
      if (error) {
        ...
        if (type === 'xml') {
            window.dispatchEvent(new CustomEvent('wbgImportStep', { detail: { step: 'csv_base' } }));
        } else if (type === 'csv_base') {
            window.dispatchEvent(new CustomEvent('wbgImportStep', { detail: { step: 'csv_new' } }));
        } else if (type === 'csv_new') {
            window.dispatchEvent(new CustomEvent('wbgImportStep', { detail: { step: 'done' } }));
        }
      }
    } catch (err) {
      console.error(err);
