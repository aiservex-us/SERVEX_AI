...
export default function TeamsAgentChat({ currentSection, renderTool, onOpenToolPanel }) {
  ...
  useEffect(() => {
    ...
    const handleWbgImportStep = (e) => {
        const { step } = e.detail;
        if (step === 'csv_base') {
            setMessages(prev => [...prev, { from: 'bot', text: 'XML successfully saved. The Base CSV is already in the system. Now, please upload the Updated CSV file.', isNew: true, time: new Date().toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'}) }, { from: 'tool', toolId: 'incert_wbg_csv_new' }]);
        } else if (step === 'csv_new') {
            setMessages(prev => [...prev, { from: 'bot', text: 'Base CSV successfully saved. Finally, upload the Updated CSV file.', isNew: true, time: new Date().toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'}) }, { from: 'tool', toolId: 'incert_wbg_csv_new' }]);
        } else if (step === 'done') {
    ...
    window.addEventListener('wbgImportStep', handleWbgImportStep);
    
    return () => {
      window.removeEventListener('globalChatMessage', handleGlobalMessage);
      window.removeEventListener('wbgImportStep', handleWbgImportStep);
    };
  }, []);
...
if (queryToSend.toLowerCase() === '/importbase') {
      setTimeout(() => {
        setMessages(prev => [...prev, { from: 'bot', text: 'Please upload the master XML file for WBG.', isNew: true }, { from: 'tool', toolId: 'incert_delete' }]);
        setIsLoading(false);
        scrollToBottom(true);
      }, 500);
      return;
    ...
    if (queryToSend.toLowerCase() === '/downloadresultxml') {
      ...
      try {
        ...
        if (error || !data || !data.xml_actualizer_raw) {
          ...
          link.href = url;
          link.setAttribute('download', 'WBG.xml');
    ...
    if (queryToSend === '/executeProcess') {
      ...
      try {
        ...
        formData.append('company_name', 'WBG');
        
        const response = await fetch(`${apiURL}/wbg/api/v1/pipeline/compare-only-WBG`, {
          method: 'POST',
          body: formData,
        });
          ...
          {messages.length > 0 && (
            <button
              onClick={async () => {
                setMessages([]);
                try {
                  await fetch(`${apiURL}/wbg/api/v1/agent/history`, {method: "DELETE" });
                } catch (e) {}
              }}
