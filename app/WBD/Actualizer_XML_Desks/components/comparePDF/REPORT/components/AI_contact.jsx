...
const QUICK_PROMPTS = [
  {icon: BarChart2, label: "Desks Analysis", q: "Generate a summary of the most critical price variations in the Desks catalog." },
  {icon: Database, label: "WBD Integrity", q: "Verify the structure and alignment of the WBD master XML." },
];
...
export default function TeamsAgentChat({ currentSection, renderTool, onOpenToolPanel }) {
  ...
  useEffect(() => {
    ...
    const handleWBDImportStep = (e) => {
        const { step } = e.detail;
        if (step === 'csv_base') {
            setMessages(prev => [...prev, { from: 'bot', text: 'XML guardado exitosamente. Ahora, por favor sube el archivo CSV Base.', isNew: true, time: new Date().toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'}) }, { from: 'tool', toolId: 'incert_wbd_csv_base' }]);
        } else if (step === 'csv_new') {
            setMessages(prev => [...prev, { from: 'bot', text: 'CSV Base guardado exitosamente. Finalmente, sube el archivo CSV Actualizado.', isNew: true, time: new Date().toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'}) }, { from: 'tool', toolId: 'incert_wbd_csv_new' }]);
        } else if (step === 'done') {
    ...
    window.addEventListener('wbdImportStep', handleWBDImportStep);

    return () => {
      window.removeEventListener('globalChatMessage', handleGlobalMessage);
      window.removeEventListener('wbdImportStep', handleWBDImportStep);
    };
  }, []);
...
if (queryToSend.toLowerCase() === '/importbase') {
      setTimeout(() => {
        setMessages(prev => [...prev, { from: 'bot', text: 'Por favor, sube el archivo XML maestro de WBD.', isNew: true }, { from: 'tool', toolId: 'incert_delete' }]);
        setIsLoading(false);
        scrollToBottom(true);
      }, 500);
      return;
    ...
    if (queryToSend.toLowerCase() === '/downloadresultxml') {
      ...
      try {
        ...
        if (error || !data || !data.xml_actualizer_raw) {
          setMessages(prev => [...prev, {from: "bot", text: "❌ Error: XML file not found in the database for WBD.", time: nowTime }]);
        } else {
          ...
          setMessages(prev => [...prev, {from: "bot", text: "✅ Download started. The file WBD.xml has been saved successfully.", time: nowTime }]);
        }
      } catch (err) {
    ...
    if (queryToSend === '/executeProcess') {
      const nowTime = new Date().toLocaleTimeString([], {hour: '2-digit', minute: '2-digit' });
      setMessages(prev => [...prev, {from: "bot", text: "⚙️ Starting ETL engine for cloud catalog processing (WBD). Please wait...", time: nowTime }]);
      
      try {
        const formData = new FormData();
        formData.append('company_name', 'WBD');
        
        const response = await fetch(`${apiURL}/wbd/api/v1/pipeline/compare-only-WBD`, {
          method: 'POST',
          body: formData,
        });
