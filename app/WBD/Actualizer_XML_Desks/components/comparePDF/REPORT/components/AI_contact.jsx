    ...
    if (queryToSend.toLowerCase() === '/downloadresultxml') {
      ...
      try {
        ...
        if (error || !data || !data.xml_actualizer_raw) {
          setMessages(prev => [...prev, { from: "bot", text: "❌ Error: No se encontró el file XML en la base de datos para WBD.", time: nowTime }]);
        } else {
          ...
          setMessages(prev => [...prev, { from: "bot", text: "✅ Descarga iniciada. El file WBD.xml ha sido guardado exitosamente.", time: nowTime }]);
        }
      } catch (err) {
