...
const QUICK_PROMPTS = [
  {icon: BarChart2, label: "Seatings Analysis", q: "Generate a summary of the most critical price variations in the Seatings catalog." },
  {icon: Database, label: "WBS Integrity", q: "Verify the structure and alignment of the WBS master XML." },
];
...
export default function TeamsAgentChat({ currentSection, renderTool, onOpenToolPanel }) {
  ...
  useEffect(() => {
    ...
    const handleWBSImportStep = (e) => {
        const { step } = e.detail;
        if (step === 'csv_base') {
            setMessages(prev => [...prev, { from: 'bot', text: 'XML guardado exitosamente. Ahora, por favor sube el archivo CSV Base.', isNew: true, time: new Date().toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'}) }, { from: 'tool', toolId: 'incert_wbs_csv_base' }]);
