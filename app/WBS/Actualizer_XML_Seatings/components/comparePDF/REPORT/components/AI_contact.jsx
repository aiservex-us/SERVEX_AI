...
export default function TeamsAgentChat({ currentSection }) {
  ...
  const sendMessage = async (overrideText) => {
    ...
    if (queryToSend.toLowerCase() === '/downloadresultxml') {
      ...
      try {
        ...
        if (error || !data || !data.xml_actualizer_raw) {
          setMessages(prev => [...prev, { from: "bot", text: "❌ Error: No se encontró el archivo XML en la base de datos para WBS.", time: nowTime }]);
        } else {
          ...
          setMessages(prev => [...prev, { from: "bot", text: "✅ Descarga iniciada. El archivo WBS.xml ha sido guardado exitosamente.", time: nowTime }]);
        }
      } catch (err) {
    ...
    if (queryToSend === '/executeProcess') {
      const nowTime = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
      setMessages(prev => [...prev, { from: "bot", text: "⚙️ Iniciando motor ETL para procesamiento de catálogos en la nube (WBS). Por favor, espera...", time: nowTime }]);
