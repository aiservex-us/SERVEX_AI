...
const Header GroupDataMatrix = () => {
  ...
  const processXML = async () => {
    try {
      ...
      console.error("Error processing Header Group data matrix:", err);
      setError(err.message || "Error processing catalog information Header Group.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute('download', `Header Group_XML_Results_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(workbook, `Header Group_XML_Results_${new Date().toISOString().slice(0,10)}.xlsx`);
  };
  ...
  if (loading) return (
    <div className="flex items-center justify-center min-h-[90vh] bg-white text-xs font-semibold text-slate-500 font-sans">
      <div className="flex items-center gap-2">
        ...
        Retrieving master data matrix from Header Group Engine...
      </div>
    </div>
  );
        ...
        <div className="bg-white/90 backdrop-blur-xl rounded-2xl border border-white shadow-2xl shadow-[#464775]/10 overflow-hidden flex flex-col w-full">
          ...
          <div className="px-4 py-2 border-b border-slate-100 bg-gradient-to-r from-slate-50/40 to-white flex flex-col md:flex-row md:items-center justify-between gap-3">
            <div className="flex flex-col">
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold text-slate-800">Header Group Seatings XML Matrix (Master Format)</span>
