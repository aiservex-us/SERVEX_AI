...
export default function TeamsAgentChat({ currentSection, renderTool, onOpenToolPanel }) {
  ...
  useEffect(() => {
    ...
    const handleHeader GroupImportStep = (e) => {
        const { step } = e.detail;
        if (step === 'csv_base') {
    ...
    window.addEventListener('wbsImportStep', handleHeader GroupImportStep);

    return () => {
      window.removeEventListener('globalChatMessage', handleGlobalMessage);
      window.removeEventListener('wbsImportStep', handleHeader GroupImportStep);
    };
  }, []);
