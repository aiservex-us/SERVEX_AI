...
export default function CriticalExcelModal({ xmlRoute, moduleName = 'default' }) {
  ...
  const isWBT = moduleName === 'WBT';
  const theme = {
    primaryBg: isWBT ? 'bg-[#2563eb]' : '${theme.primaryBg}',
    primaryBgHover: isWBT ? 'hover:bg-[#1d4ed8]' : '${theme.primaryBgHover}',
    primaryText: isWBT ? 'text-[#2563eb]' : '${theme.primaryText}',
    ...
    primaryShadow: isWBT ? 'shadow-[#2563eb]/25' : '${theme.primaryShadow}',
    primaryBorderHover: isWBT ? 'hover:border-[#2563eb]/30' : '${theme.primaryBorderHover}',
    primaryFillBg: isWBT ? 'bg-[#2563eb]/10' : '${theme.primaryBg}/10',
    primaryFillBgAlt: isWBT ? 'bg-[#2563eb]/5' : '${theme.primaryBg}/5',
    primaryBorder: isWBT ? 'border-[#2563eb]/10' : '${theme.primaryBorder}',
    shapeStyle: isWBT 
          ...
          >
            ...
            <div className={`hidden md:flex w-[40%] relative items-center justify-center overflow-hidden bg-gradient-to-b ${theme.primaryGradient} to-white p-10 border-r border-slate-100`}>
              ...
              <div className="relative z-20 text-[#2B2C4B] mt-auto w-full">
                ...
                <ShieldCheck size={32} className={`${theme.primaryText} mb-3`} />
            ...
            <div className="w-full md:w-[60%] p-8 sm:p-10 flex flex-col justify-center bg-[#fcfcfd] relative">
              <button 
                ...
                className="absolute top-6 right-6 p-2 text-slate-400 hover:${theme.primaryText} hover:bg-slate-100 rounded-full transition-colors"
              >
                <X size={20} />
              </button>

              <div className="mb-8">
                <span className="inline-flex items-center gap-2 rounded-full ${theme.primaryBg}/5 px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider ${theme.primaryText} mb-4 border ${theme.primaryBorder}">
                  <span className={`w-1.5 h-1.5 rounded-full ${theme.primaryBg} animate-pulse`} />
                  Action Required
                </span>
              ...
              <div className="space-y-4 mb-10">
                {/* Feature 1 */}
                <div className={`group flex gap-4 items-start p-4 bg-white rounded-2xl border border-slate-100 shadow-sm ${theme.primaryBorderHover} hover:shadow-md transition-all duration-300 cursor-default`}>
                  <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${theme.primaryFillBg} to-${theme.primaryFillBgAlt.replace('bg-', '')} flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform duration-300`}>
                    <FileCode className={`w-6 h-6 ${theme.primaryText}`} strokeWidth={1.5} />
                  </div>
                  <div>
