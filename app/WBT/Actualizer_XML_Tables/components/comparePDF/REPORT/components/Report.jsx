      ...
      <div className="w-full max-w-[90vw] mx-auto">
        ...
        {metadataRaw && (
          <div className="mb-4 bg-[#F9FAFB] rounded-md border border-[#E0E0E0] p-3 text-xs flex flex-wrap items-center gap-6 justify-between">
            ...
            <div className="text-[11px] font-semibold text-[#5B5FC7] bg-[#EFEEFC] px-2 py-0.5 rounded">
              Target: {reportDataP?.pipeline_metadata?.execution_target || 'WBT Engine'}
            </div>
          </div>
        )}
        ...
        <div className="mb-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          <div className="bg-white border border-[#E0E0E0] rounded-md p-3 shadow-[0_2px_4px_rgba(0,0,0,0.02)] flex items-start gap-3">
            ...
            <div>
              <p className="text-[9px] font-bold text-[#616161] uppercase tracking-wider">Total Evaluated</p>
              ...
              <p className="text-[10px] text-[#A6A6A6]">Cross-matrix models</p>
            </div>
          </div>
          ...
          <div className="bg-white border border-[#E0E0E0] rounded-md p-3 shadow-[0_2px_4px_rgba(0,0,0,0.02)] flex items-start gap-3">
            ...
            <div>
              <p className="text-[9px] font-bold text-[#616161] uppercase tracking-wider">Changes Detected</p>
              <p className="text-base font-bold text-[#242424]">{summaryRaw?.cell_changes_detected_count || metricsP?.total_cell_changes || 0}</p>
              <p className="text-[10px] text-[#107C10] font-semibold">✓ {metricsP?.xml_successful_updates || 0} Injected into XML</p>
            </div>
          </div>
          ...
          <div className="bg-white border border-[#E0E0E0] rounded-md p-3 shadow-[0_2px_4px_rgba(0,0,0,0.02)] flex items-start gap-3">
            ...
            <div>
              <p className="text-[9px] font-bold text-[#616161] uppercase tracking-wider">Deleted Models</p>
              <p className="text-base font-bold text-[#A4262C]">{summaryRaw?.deleted_models_detected_count || 0}</p>
              <p className="text-[10px] text-[#616161]">Removed from source</p>
            </div>
          </div>
          ...
          <div className="bg-white border border-[#E0E0E0] rounded-md p-3 shadow-[0_2px_4px_rgba(0,0,0,0.02)] flex items-start gap-3">
            ...
            <div>
              <p className="text-[9px] font-bold text-[#616161] uppercase tracking-wider">New Models</p>
              <p className="text-base font-bold text-[#107C10]">{summaryRaw?.new_models_detected_count || 0}</p>
              <p className="text-[10px] text-[#616161]">New SKU entries</p>
            </div>
          </div>
        </div>
            ...
            <div className="w-full overflow-x-auto">
              <table className="table-fixed border-collapse text-left text-xs w-full">
                <thead className="bg-[#FDFDFD]">
                  <tr>
                    {['#', 'Model ID', 'Node / Column', 'Original Value', 'New Value', '% Diff'].map(h => (
                ...
                <tbody className="divide-y divide-[#F0F0F0]">
                  {changesP.map((c, i) => {
                    ...
                    return (
                      <tr key={i} className="hover:bg-[#F7F5FA]">
                        <td className="px-3 py-2 text-[10px] text-[#A6A6A6] font-mono w-12">{i + 1}</td>
                  ...
                  {changesP.length === 0 && (
                    <tr>
                      <td colSpan={6} className="text-center py-8 text-[#616161]">No changes registered in List Price cells.</td>
                    </tr>
                  )}
                </tbody>
              </table>
            ...
            <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-6 bg-[#FCFCFC]">
              ...
              <div className="bg-white border border-[#E0E0E0] rounded-sm shadow-sm overflow-hidden">
                <div className="bg-[#F1F9F1] px-3 py-2 border-b border-[#E0E0E0] flex items-center gap-2">
                  ...
                  <span className="text-[11px] font-bold text-[#107C10] uppercase tracking-wider">New Models Detected in Source</span>
                </div>
                  ...
                  {summaryRaw?.new_models_list && summaryRaw.new_models_list.length > 0 ? (
                    summaryRaw.new_models_list.map((model, idx) => (
                      <div key={idx} className="py-2 px-1 flex items-center justify-between font-mono text-xs hover:bg-[#F9FAFB]">
                        ...
                        <span className="text-[10px] text-[#107C10] bg-[#E1F4E1] px-1.5 py-0.5 rounded font-sans font-medium">New SKU</span>
                      </div>
                    ))
                  ) : (
                    <p className="text-xs text-[#616161] p-4 text-center">No new models introduced detected.</p>
                  )}
                </div>
              </div>
              ...
              <div className="bg-white border border-[#E0E0E0] rounded-sm shadow-sm overflow-hidden">
                <div className="bg-[#FDF3F3] px-3 py-2 border-b border-[#E0E0E0] flex items-center gap-2">
                  ...
                  <span className="text-[11px] font-bold text-[#A4262C] uppercase tracking-wider">Removed / Not Found Models</span>
                </div>
                <div className="p-2 max-h-[400px] overflow-y-auto divide-y divide-[#F3F2F1]">
                  {summaryRaw?.deleted_models_list && summaryRaw.deleted_models_list.length > 0 ? (
                    summaryRaw.deleted_models_list.map((model, idx) => (
                      <div key={idx} className="py-2 px-1 flex items-center justify-between font-mono text-xs hover:bg-[#F9FAFB]">
                        ...
                        <span className="text-[10px] text-[#A4262C] bg-[#FDE7E9] px-1.5 py-0.5 rounded font-sans font-medium">Out of Catalog</span>
                      </div>
                    ))
                  ) : (
                    <p className="text-xs text-[#616161] p-4 text-center">No models removed from catalog detected.</p>
                  )}
                </div>
              </div>
