...
const QUICK_PROMPTS = [
  {icon: BarChart2, label: "Tables Analysis", q: "Generate a summary of the most critical price variations in the Tables catalog." },
  {icon: Database, label: "WBT Integrity", q: "Verify the structure and alignment of the WBT master XML." },
];
...
if (queryToSend.toLowerCase() === '/importbase') {
    ...
    if (queryToSend.toLowerCase() === '/downloadresultxml') {
      ...
      try {
        ...
        if (error || !data || !data.xml_actualizer_raw) {
          setMessages(prev => [...prev, {from: "bot", text: "❌ Error: XML file not found in the database for WBT.", time: nowTime }]);
        } else {
          ...
          setMessages(prev => [...prev, {from: "bot", text: "✅ Download started. The file WBT.xml has been saved successfully.", time: nowTime }]);
        }
      } catch (err) {
    ...
    if (queryToSend === '/executeProcess') {
      const nowTime = new Date().toLocaleTimeString([], {hour: '2-digit', minute: '2-digit' });
      setMessages(prev => [...prev, {from: "bot", text: "⚙️ Starting ETL engine for cloud catalog processing (WBT). Please wait...", time: nowTime }]);
          ...
          {messages.length > 0 && (
            <button
              onClick={async () => {
                setMessages([]);
                try {
                  await fetch(`${apiURL}/wbt/api/v1/agent/history`, {method: "DELETE" });
                } catch (e) {}
              }}
