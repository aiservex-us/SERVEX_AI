...
export default function TeamsAgentChat({ currentSection, renderTool, onOpenToolPanel }) {
  ...
  useEffect(() => {
    ...
    const handleDALS LightingImportStep = (e) => {
        const { step } = e.detail;
        if (step === 'csv_base') {
    ...
    window.addEventListener('wbsImportStep', handleDALS LightingImportStep);

    return () => {
      window.removeEventListener('globalChatMessage', handleGlobalMessage);
      window.removeEventListener('wbsImportStep', handleDALS LightingImportStep);
    };
  }, []);
