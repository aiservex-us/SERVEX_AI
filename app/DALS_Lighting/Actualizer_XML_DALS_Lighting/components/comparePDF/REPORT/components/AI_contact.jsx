...
export default function TeamsAgentChat({ currentSection, renderTool, onOpenToolPanel }) {
  ...
  useEffect(() => {
    const checkDbPhase = async () => {
      try {
        const moduleMatch = window.location.pathname.match(/^\/(DALS Lighting[A-Z]|LESRO)/i);
        const modName = moduleMatch ? moduleMatch[1].toUpperCase() : 'DALS Lighting';
  ...
  useEffect(() => {
    ...
    const handleDALS_LightingImportStep = (e) => {
        const { step } = e.detail;
        if (step === 'csv_base') {
            ...
            setMessages(prev => [...prev, { from: 'bot', text: 'CSV Base guardado exitosamente. Finalmente, sube el archivo CSV Actualizado.', isNew: true, time: new Date().toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'}) }, { from: 'tool', toolId: 'incert_wbs_csv_new' }]);
        } else if (step === 'done') {
    ...
    window.addEventListener('wbsImportStep', handleDALS_LightingImportStep);

    return () => {
      window.removeEventListener('globalChatMessage', handleGlobalMessage);
      window.removeEventListener('wbsImportStep', handleDALS_LightingImportStep);
    };
  }, []);
...
if (queryToSend.toLowerCase() === '/importbase') {
      setTimeout(() => {
        setMessages(prev => [...prev, { from: 'bot', text: 'Por favor, sube el archivo XML maestro de DALS Lighting.', isNew: true }, { from: 'tool', toolId: 'incert_delete' }]);
        setIsLoading(false);
        scrollToBottom(true);
      }, 500);
      return;
    ...
    if (queryToSend.toLowerCase() === '/downloadresultxml') {
      ...
      try {
        const {data, error } = await supabase
          .from('ClientsSERVEX_DALS_Lighting')
          .select('xml_actualizer_raw')
          .eq('company_name', 'DALS Lighting')
          .single();

        if (error || !data || !data.xml_actualizer_raw) {
          setMessages(prev => [...prev, {from: "bot", text: "❌ Error: XML file not found in the database for DALS Lighting.", time: nowTime }]);
        } else {
          const blob = new Blob([data.xml_actualizer_raw], {type: 'application/xml' });
          const url = window.URL.createObjectURL(blob);
          const link = document.createElement('a');
          link.href = url;
          link.setAttribute('download', 'DALS Lighting.xml');
          document.body.appendChild(link);
          link.click();
          link.parentNode.removeChild(link);
          window.URL.revokeObjectURL(url);
          
          setMessages(prev => [...prev, {from: "bot", text: "✅ Download started. The file DALS Lighting.xml has been saved successfully.", time: nowTime }]);
        }
      } catch (err) {
    ...
    if (queryToSend === '/createAuditor') {
      ...
      try {
        const match = window.location.pathname.match(/^\/(DALS Lighting[A-Z])/i);
    ...
    if (queryToSend === '/executeProcess') {
      const nowTime = new Date().toLocaleTimeString([], {hour: '2-digit', minute: '2-digit' });
      setMessages(prev => [...prev, {from: "bot", text: "⚙️ Starting ETL engine for cloud catalog processing (DALS Lighting). Please wait...", time: nowTime }]);
      
      try {
        const formData = new FormData();
        formData.append('company_name', 'DALS Lighting');
        
        const response = await fetch(`${apiURL}/wbs/api/v1/pipeline/compare-only-DALS Lighting`, {
          method: 'POST',
          body: formData,
        });
