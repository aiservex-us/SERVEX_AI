...
import AIReporting from './components/comparePDF/presentation_DALS_Lighting.jsx'
...
import CETComparator from '../Actualizer_Excel_DALS_Lighting/components/CET_Comparator.jsx';
  ...
  useEffect(() => {
    const checkDelegation = async () => {
      try {
        ...
        if (!user) {
          window.location.href = '/DALS Lighting';
          return;
        }
        
        const apiURL = process.env.NEXT_PUBLIC_API_URL || 'https://servex-ai-back.onrender.com';
        const res = await fetch(`${apiURL}/api/v1/module_delegation/DALS Lighting`);
              ...
              <TeamsAgentChat 
                ...
                renderTool={(toolId) => {
                  switch (toolId) {
                    ...
                    case 'incert_wbs_csv_base': return <IncertDelete step="csv_base" />;
                    case 'incert_wbs_csv_new': return <IncertDelete step="csv_new" />;
                    case 'import_cet_xml': return <div className="w-full h-full overflow-y-auto"><ImportCETXml moduleName="DALS Lighting" /></div>;
