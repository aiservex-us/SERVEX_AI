...
export default function TeamsAgentChat({ currentSection, renderTool, onOpenToolPanel }) {
  ...
  useEffect(() => {
    const checkDbPhase = async () => {
      try {
        const moduleMatch = window.location.pathname.match(/^\/(Via Seating[A-Z]|LESRO)/i);
        const modName = moduleMatch ? moduleMatch[1].toUpperCase() : 'Via Seating';
  ...
  useEffect(() => {
    ...
    const handleVia_SeatingImportStep = (e) => {
        const { step } = e.detail;
        if (step === 'csv_base') {
    ...
    window.addEventListener('wbsImportStep', handleVia_SeatingImportStep);

    return () => {
      window.removeEventListener('globalChatMessage', handleGlobalMessage);
      window.removeEventListener('wbsImportStep', handleVia_SeatingImportStep);
    };
  }, []);
...
if (queryToSend.toLowerCase() === '/importbase') {
      setTimeout(() => {
        setMessages(prev => [...prev, { from: 'bot', text: 'Por favor, sube el archivo XML maestro de Via Seating.', isNew: true }, { from: 'tool', toolId: 'incert_delete' }]);
        setIsLoading(false);
        scrollToBottom(true);
      }, 500);
      return;
    ...
    if (queryToSend.toLowerCase() === '/downloadresultxml') {
      ...
      try {
        const {data, error } = await supabase
          .from('ClientsSERVEX_Via_Seating')
          .select('xml_actualizer_raw')
          .eq('company_name', 'Via Seating')
          .single();

        if (error || !data || !data.xml_actualizer_raw) {
          setMessages(prev => [...prev, {from: "bot", text: "❌ Error: XML file not found in the database for Via Seating.", time: nowTime }]);
        } else {
          const blob = new Blob([data.xml_actualizer_raw], {type: 'application/xml' });
          const url = window.URL.createObjectURL(blob);
          const link = document.createElement('a');
          link.href = url;
          link.setAttribute('download', 'Via Seating.xml');
          document.body.appendChild(link);
          link.click();
          link.parentNode.removeChild(link);
          window.URL.revokeObjectURL(url);
          
          setMessages(prev => [...prev, {from: "bot", text: "✅ Download started. The file Via Seating.xml has been saved successfully.", time: nowTime }]);
        }
      } catch (err) {
    ...
    if (queryToSend === '/createAuditor') {
      ...
      try {
        const match = window.location.pathname.match(/^\/(Via Seating[A-Z])/i);
    ...
    if (queryToSend === '/executeProcess') {
      const nowTime = new Date().toLocaleTimeString([], {hour: '2-digit', minute: '2-digit' });
      setMessages(prev => [...prev, {from: "bot", text: "⚙️ Starting ETL engine for cloud catalog processing (Via Seating). Please wait...", time: nowTime }]);
      
      try {
        const formData = new FormData();
        formData.append('company_name', 'Via Seating');
        
        const response = await fetch(`${apiURL}/wbs/api/v1/pipeline/compare-only-Via Seating`, {
          method: 'POST',
          body: formData,
        });
