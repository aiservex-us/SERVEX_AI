...
const Via_SeatingDataMatrix = () => {
    ...
    try {
      ...
      for (const p of productsXML) {
        ...
        const createRow = (baseSku, optSuffixCode, optPrice) => {
          ...
          const row = {};
          Via_Seating_HEADERS.forEach(h => row[h] = ""); // Initialize empty string
          ...
          Object.keys(productOptionPrices).forEach(optCode => {
            const matchingHeader = Via_Seating_HEADERS.find(h => h.includes(`(${optCode})`) || h.includes(`-${optCode}`));
            if (matchingHeader) {
              row[matchingHeader] = productOptionPrices[optCode];
            } else if (optCode.includes('MB')) {
               const mbHeader = Via_Seating_HEADERS.find(h => h.includes("(__MB)"));
      ...
      console.error("Error processing Via Seating data matrix:", err);
      setError(err.message || "Error processing catalog information Via Seating.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute('download', `Via Seating_XML_Results_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const exportToExcel = () => {
    ...
    const worksheet = XLSX.utils.json_to_sheet(filtered, { header: Via_Seating_HEADERS });
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Catalog Data");
    
    XLSX.writeFile(workbook, `Via Seating_XML_Results_${new Date().toISOString().slice(0,10)}.xlsx`);
  };
  ...
  if (loading) return (
    <div className="flex items-center justify-center min-h-[90vh] bg-white text-xs font-semibold text-slate-500 font-sans">
      <div className="flex items-center gap-2">
        ...
        Retrieving master data matrix from Via Seating Engine...
      </div>
    </div>
  );
        ...
        <div className="bg-white/90 backdrop-blur-xl rounded-2xl border border-white shadow-2xl shadow-[#464775]/10 overflow-hidden flex flex-col w-full">
          ...
          <div className="px-4 py-2 border-b border-slate-100 bg-gradient-to-r from-slate-50/40 to-white flex flex-col md:flex-row md:items-center justify-between gap-3">
            <div className="flex flex-col">
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold text-slate-800">Via Seating Seatings XML Matrix (Master Format)</span>
              ...
              <table className="table-fixed border-collapse overflow-hidden overflow-hidden text-left text-xs w-max min-w-[2000px]">
                <thead className="sticky top-0 z-20 shadow-[0_1px_0_0_#E0E0E0]">
                  <tr>
                    ...
                    {Via_Seating_HEADERS.map((header, i) => (
                      <th
                        key={header + i}
          ...
          <div className="bg-gradient-to-r from-slate-50/40 to-white px-4 py-2 border-t border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-4 text-[10px] font-semibold text-slate-500 select-none">
            <div className="flex gap-4">
              <span className="uppercase tracking-tight">TOTAL COLUMNS: {Via_Seating_HEADERS.length}</span>
...
export default Via_SeatingDataMatrix;
