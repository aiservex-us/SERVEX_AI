...
const Forum = () => {
  ...
  const renderMarkdown = (content) => (
    <ReactMarkdown
      components={{
        ...
        table: ({node, ...props}) => (
          <div className="w-full overflow-x-auto my-4 rounded-lg border border-slate-200 shadow-sm bg-white">
  ...
  return (
    <div className="w-full h-full flex flex-col bg-slate-50 font-sans overflow-hidden">
      
      {/* HEADER NAVBAR */}
      <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-8 shrink-0 shadow-sm z-10">
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 bg-gradient-to-br from-[#464775] to-[#2B2C4B] rounded-lg flex items-center justify-center shadow-sm">
        ...
        <div className="flex-1 max-w-xl mx-8 relative hidden md:block">
          ...
          <input
            type="text"
            className="block w-full pl-9 pr-3 py-2 border border-slate-200 rounded-full text-xs bg-slate-50 focus:outline-none focus:ring-2 focus:ring-[#464775]/20 focus:border-[#464775]/50 transition-all text-slate-700 placeholder-slate-400"
        ...
        <div className="flex items-center gap-3">
          <button className="p-2 text-slate-400 hover:bg-slate-100 hover:text-[#464775] rounded-full transition-colors relative">
            <Bell className="w-5 h-5" />
            <span className="absolute top-1 right-1 w-2 h-2 bg-red-500 rounded-full border border-white"></span>
          </button>
          <button onClick={fetchAuditData} className="p-2 text-slate-400 hover:bg-slate-100 hover:text-[#464775] rounded-full transition-colors">
            <RefreshCw className={`w-5 h-5 ${loading ? 'animate-spin' : ''}`} />
          </button>
          <div className="w-9 h-9 rounded-full bg-slate-200 border border-slate-200 ml-2 overflow-hidden shadow-sm">
      ...
      <main className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-transparent">
        <div className="w-full max-w-3xl mx-auto py-8 px-4 flex flex-col gap-6">
          ...
          <div className="bg-white rounded-xl p-4 shadow-sm border border-slate-200 flex gap-3">
             ...
             <div className="flex-1">
                <div className="w-full h-10 bg-slate-50 rounded-full border border-slate-200 flex items-center px-4 text-sm text-slate-400 cursor-not-allowed">
          ...
          {loading ? (
            ...
            MODULES_CONFIG.map((module) => {
              ...
              return (
                <article key={module.key} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                  ...
                  {/* Post Body (Markdown) */}
                  <div className="px-5 py-2 text-[13px] text-slate-700">
                    <div className="inline-flex items-center gap-1.5 bg-indigo-50 text-[#464775] px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider mb-4 border border-indigo-100">
                      {module.title}
                    </div>
                  ...
                  <div className="px-5 py-3 bg-white border-t border-slate-100 flex items-center justify-between">
                    <div className="flex gap-1">
                      <button className="flex items-center gap-2 px-3 py-1.5 text-slate-500 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors text-xs font-semibold">
                        <ThumbsUp className="w-4 h-4" /> Like
                      </button>
                      <button 
                        onClick={() => toggleComments(module.key)}
                        className={`flex items-center gap-2 px-3 py-1.5 rounded-lg transition-colors text-xs font-semibold ${isCommentsOpen ? 'text-[#464775] bg-indigo-50' : 'text-slate-500 hover:text-[#464775] hover:bg-indigo-50'}`}
                      >
                        <MessageSquare className="w-4 h-4" /> Comment {moduleComments.length > 0 && <span className="bg-[#464775] text-white px-1.5 py-0.5 rounded-full text-[9px] font-bold">{moduleComments.length}</span>}
                      </button>
                      <button className="flex items-center gap-2 px-3 py-1.5 text-slate-500 hover:text-green-600 hover:bg-green-50 rounded-lg transition-colors text-xs font-semibold">
                  ...
                  {/* COMMENTS SECTION */}
                  {isCommentsOpen && (
                    <div className="bg-slate-50 border-t border-slate-100 flex flex-col p-5">
                        ...
                        {moduleComments.length === 0 ? (
                          ...
                          moduleComments.map((comment) => (
                            <div key={comment.id} className="flex gap-3">
                              <img src={comment.avatar} alt="Avatar" className="w-8 h-8 rounded-full shadow-sm border border-slate-200" />
                      ...
                      <div className="flex items-end gap-3 mt-auto">
                        ...
                        <div className="flex-1 bg-white border border-slate-200 rounded-[1.25rem] shadow-sm p-1.5 flex items-center focus-within:border-[#464775]/50 focus-within:ring-2 focus-within:ring-[#464775]/10 transition-all">
                          <input
                            type="text"
                          ...
                          <button 
                            ...
                            className="w-8 h-8 rounded-full bg-[#464775] text-white flex items-center justify-center hover:bg-[#34355a] transition-colors disabled:opacity-50 disabled:cursor-not-allowed shrink-0 shadow-sm"
                          >
