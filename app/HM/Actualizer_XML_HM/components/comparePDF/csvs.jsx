...
export default function DataViewer() {
  ...
  const fetchLatestData = async () => {
    setLoading(true);
    try {
      // Consulta corregida alineada estrictamente al DDL provisto para la entidad H&M
      ...
      console.error('Error fetching H&M data:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDownloadCSV = () => {
    ...
    link.href = url;
    const filename = `${data?.company_name || 'H&M'}_${activeTab === 'csv_raw' ? 'Sanitized_Manual' : 'Sanitized_PDF'}_${new Date().toISOString().slice(0,10)}.csv`;
  ...
  if (loading) return (
    <div className="flex items-center justify-center min-h-[90vh] bg-white text-xs font-semibold text-slate-500 font-sans">
      <div className="flex items-center gap-2">
        ...
        Retrieving master data matrix for H&M...
      </div>
    </div>
  );
