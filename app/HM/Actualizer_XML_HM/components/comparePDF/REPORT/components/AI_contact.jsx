...
export default function TeamsAgentChat({ currentSection, renderTool, onOpenToolPanel }) {
  ...
  useEffect(() => {
    const checkDbPhase = async () => {
      try {
        const moduleMatch = window.location.pathname.match(/^\/(H&M[A-Z]|LESRO)/i);
        const modName = moduleMatch ? moduleMatch[1].toUpperCase() : 'H&M';
  ...
  useEffect(() => {
    ...
    window.addEventListener('wbsImportStep', handleH&MImportStep);

    return () => {
      window.removeEventListener('globalChatMessage', handleGlobalMessage);
      window.removeEventListener('wbsImportStep', handleH&MImportStep);
    };
  }, []);
...
if (queryToSend.toLowerCase() === '/importbase') {
      setTimeout(() => {
        setMessages(prev => [...prev, { from: 'bot', text: 'Por favor, sube el archivo XML maestro de H&M.', isNew: true }, { from: 'tool', toolId: 'incert_delete' }]);
        setIsLoading(false);
        scrollToBottom(true);
      }, 500);
      return;
    ...
    if (queryToSend.toLowerCase() === '/downloadresultxml') {
      ...
      try {
        ...
        if (error || !data || !data.xml_actualizer_raw) {
          setMessages(prev => [...prev, {from: "bot", text: "❌ Error: XML file not found in the database for H&M.", time: nowTime }]);
        } else {
          const blob = new Blob([data.xml_actualizer_raw], {type: 'application/xml' });
          const url = window.URL.createObjectURL(blob);
          const link = document.createElement('a');
          link.href = url;
          link.setAttribute('download', 'H&M.xml');
          document.body.appendChild(link);
          link.click();
          link.parentNode.removeChild(link);
          window.URL.revokeObjectURL(url);
          
          setMessages(prev => [...prev, {from: "bot", text: "✅ Download started. The file H&M.xml has been saved successfully.", time: nowTime }]);
        }
      } catch (err) {
    ...
    if (queryToSend === '/createAuditor') {
      ...
      try {
        const match = window.location.pathname.match(/^\/(H&M[A-Z])/i);
    ...
    if (queryToSend === '/executeProcess') {
      const nowTime = new Date().toLocaleTimeString([], {hour: '2-digit', minute: '2-digit' });
      setMessages(prev => [...prev, {from: "bot", text: "⚙️ Starting ETL engine for cloud catalog processing (H&M). Please wait...", time: nowTime }]);
      
      try {
        const formData = new FormData();
        formData.append('company_name', 'H&M');
        
        const response = await fetch(`${apiURL}/wbs/api/v1/pipeline/compare-only-H&M`, {
          method: 'POST',
          body: formData,
        });
