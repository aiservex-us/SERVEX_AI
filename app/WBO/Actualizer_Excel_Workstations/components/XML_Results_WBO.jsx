...
const WBODataMatrix = () => {
    ...
    try {
      ...
      for (const p of productsXML) {
        ...
        const createRow = (baseSku, optSuffixCode, optPrice) => {
          ...
          const row = {};
          WBO_HEADERS.forEach(h => row[h] = ""); // Initialize empty string

          // Map base fields
          row["Model #"] = finalSku;
          row["2026 List Price"] = finalPrice;
          ...
          Object.keys(productOptionPrices).forEach(optCode => {
            const matchingHeader = WBO_HEADERS.find(h => h.includes(`(${optCode})`) || h.includes(`-${optCode}`));
            if (matchingHeader) {
              row[matchingHeader] = productOptionPrices[optCode];
            } else if (optCode.includes('MB')) {
               const mbHeader = WBO_HEADERS.find(h => h.includes("(__MB)"));
  ...
  const stats = useMemo(() => {
    ...
    const avgPrice = total 
      ? Math.round(products.reduce((acc, p) => acc + (p["2026 List Price"] || 0), 0) / total) 
      : 0;
  ...
  const exportToExcel = () => {
    ...
    const worksheet = XLSX.utils.json_to_sheet(filtered, { header: WBO_HEADERS });
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Catalog Data");
    
    XLSX.writeFile(workbook, `WBO_XML_Results_${new Date().toISOString().slice(0,10)}.xlsx`);
  };
        ...
        <div className="bg-white/90 backdrop-blur-xl rounded-2xl border border-white shadow-2xl shadow-[#7f1d1d]/10 overflow-hidden flex flex-col w-full">
          ...
          <div className="px-4 py-2 border-b border-slate-100 bg-gradient-to-r from-slate-50/40 to-white flex flex-col md:flex-row md:items-center justify-between gap-3">
            <div className="flex flex-col">
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold text-slate-800">WBO Workstations XML Matrix (Master Format)</span>
                    ...
                    {paginatedProducts.map((p, idx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {WBO_HEADERS.map((header) => {
                            let value = p[header];
                            if (header === "2026 List Price") value = `$${(p["2026 List Price"] || 0).toLocaleString()}`;
                            
                            return (
                              <td key={header} className={`p-0 text-slate-800 border-r border-b border-slate-50 min-w-[160px] max-w-[280px]`}>
                                <div className={`px-3 py-1.5 font-sans text-[11px] whitespace-nowrap truncate ${header === 'Model #' || header === '2026 List Price' ? 'font-bold font-mono text-[#7f1d1d]' : 'font-medium'}`} title={value}>
                                  {value}
                                </div>
                              </td>
                            );
          ...
          <div className="bg-gradient-to-r from-slate-50/40 to-white px-4 py-2 border-t border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-4 text-[10px] font-semibold text-slate-500 select-none">
            <div className="flex gap-4">
              <span className="uppercase tracking-tight">TOTAL COLUMNS: {WBO_HEADERS.length}</span>
