  ...
  const handleDropNewCSV = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    try {
      ...
      if (error) {
        ...
        if (type === 'xml') {
            window.dispatchEvent(new CustomEvent('wboImportStep', { detail: { step: 'csv_base' } }));
        } else if (type === 'csv_base') {
            window.dispatchEvent(new CustomEvent('wboImportStep', { detail: { step: 'csv_new' } }));
        } else if (type === 'csv_new') {
            window.dispatchEvent(new CustomEvent('wboImportStep', { detail: { step: 'done' } }));
        }
      }
    } catch (err) {
      console.error(err);
