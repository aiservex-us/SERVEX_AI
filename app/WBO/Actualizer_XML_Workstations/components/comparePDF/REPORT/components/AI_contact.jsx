...
export default function TeamsAgentChat({ currentSection, renderTool, onOpenToolPanel }) {
  ...
  useEffect(() => {
    ...
    const handleWBOImportStep = (e) => {
        const { step } = e.detail;
        if (step === 'csv_base') {
            setMessages(prev => [...prev, { from: 'bot', text: 'XML successfully saved. The Base CSV is already in the system. Now, please upload the Updated CSV file.', isNew: true, time: new Date().toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'}) }, { from: 'tool', toolId: 'incert_wbo_csv_new' }]);
        } else if (step === 'csv_new') {
            setMessages(prev => [...prev, { from: 'bot', text: 'Base CSV successfully saved. Finally, upload the Updated CSV file.', isNew: true, time: new Date().toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'}) }, { from: 'tool', toolId: 'incert_wbo_csv_new' }]);
        } else if (step === 'done') {
...
if (queryToSend.toLowerCase() === '/importbase') {
      setTimeout(() => {
        setMessages(prev => [...prev, { from: 'bot', text: 'Please upload the master XML file for WBO.', isNew: true }, { from: 'tool', toolId: 'incert_delete' }]);
        setIsLoading(false);
        scrollToBottom(true);
      }, 500);
      return;
