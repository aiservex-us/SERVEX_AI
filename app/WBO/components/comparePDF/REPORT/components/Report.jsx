...
export default function AuditReportViewer() {
  ...
  // 1. Inicializar en null o string vacío de forma consistente
  const [selectedRecordId, setSelectedRecordId] = useState('');

  // Función de fetch extraída para poder reutilizarla en el botón de refresh
  async function fetchAuditData() {
    setLoading(true);
    try {
      const { data, error } = await supabase
        .from('ClientsSERVEX_WBO')
        // Asegúrate de traer audit_report_jsonP exactamente como está en PostgreSQL
      ...
      if (data && data.length > 0) {
        // 2. Guardamos el ID tal como viene de la base de datos (generalmente String o Number)
  ...
  // 3. Normalizamos la comparación convirtiendo ambos lados a String
  const activeRecord = records.find(r => r.id.toString() === selectedRecordId.toString());
        ...
        <div className="bg-white rounded-md border border-[#E0E0E0] shadow-[0_2px_4px_rgba(0,0,0,0.04)] overflow-hidden flex flex-col w-full">
          ...
          <div className="px-4 py-2 border-b border-[#E0E0E0] bg-gradient-to-r from-white via-[#FCFAFF] to-[#F7F3FF] flex flex-col md:flex-row md:items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              <select 
                ...
                // 4. Mantenemos el valor como String al cambiar
                onChange={(e) => setSelectedRecordId(e.target.value)}
              >
                {records.map((rec) => <option key={rec.id} value={rec.id}>{rec.company_name}</option>)}
              </select>
              {/* Asignamos la función de refresco al botón */}
              <button onClick={fetchAuditData} className="p-1 bg-white border border-[#D2D2D2] hover:bg-[#F3F2F1] rounded-sm text-[#616161]">
                <RefreshCw size={13} />
              </button>
            </div>
          </div>
          ...
          <div className="bg-[#FDFDFD] border-b border-[#E0E0E0] p-4 grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-[10px] text-[#242424]">Estado de Sincronización</p>
          ...
          <div className="w-full overflow-x-auto">
            <table className="table-fixed border-collapse text-left text-xs w-full">
              ...
              <tbody className="divide-y divide-[#F0F0F0]">
                {/* 5. Aseguramos fallback si detected_changes no existe o está vacío */}
                {reportData?.detected_changes && reportData.detected_changes.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan={6} className="px-3 py-8 text-center text-slate-400 text-[11px]">
                      No se encontraron discrepancias o datos válidos de auditoría para este registro.
                    </td>
                  </tr>
                )}
              </tbody>
