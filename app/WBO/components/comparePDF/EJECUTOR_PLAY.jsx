...
const EJECUTOR_PLAY = ({ 
  ...
  return (
    <div className="flex flex-col gap-2 sm:gap-3 lg:gap-4 font-sans antialiased text-[#242424]">
        ...
        <div className="fixed inset-0 z-[1001] flex items-center justify-center bg-white/20 backdrop-blur-md animate-in fade-in duration-300 p-4 sm:p-6">
          <div className="bg-white border border-gray-200 shadow-2xl rounded-lg sm:rounded-2xl p-4 sm:p-6 max-w-sm w-full text-center space-y-3 sm:space-y-4 transform animate-in zoom-in-95 duration-200">
            ...
            <div className="space-y-1">
              <h3 className="text-xs sm:text-sm font-bold text-gray-800 uppercase tracking-tight">Audit Pipeline Initiated</h3>
              <p className="text-[10px] sm:text-[11px] text-gray-500 font-medium">Catalog (WBO): {currentDate}</p>
            </div>
      ...
      {/* --- ACTION CONTAINER --- */}
      <div className="bg-white border border-[#e0e0e0] rounded-lg p-5 shadow-sm flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2 pb-3 border-b border-[#f0f0f0]">
            <button 
              ...
              className="w-full bg-[#464775] hover:bg-[#4f52b2] disabled:bg-[#f0f0f0] disabled:text-[#bdbdbd] text-white py-2 px-3 rounded-md font-semibold text-[11px] transition-all flex items-center justify-center gap-2 shadow-sm"
            >
              {isProcessing ? <Loader2 size={14} className="animate-spin" /> : <FiZap size={14} />}
              RUN PIPELINE & SYNC
            </button>
            <div className="px-1 py-2">
              <p className="text-[9px] font-bold text-[#5b5fc7] mb-1 uppercase tracking-wide">Process:</p>
              <p className="text-[9px] text-[#616161] leading-relaxed">
                Initializes the multi-stage pipeline to audit CSV data against the master database. Performs delimiter detection, header normalization, discrepancy analysis, and synchronizes results to Supabase with full audit logging.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-2">
            <button 
              ...
              className="w-full py-2 px-3 text-[10px] font-medium text-[#616161] hover:text-[#c4314b] hover:bg-[#f5f5f5] border border-[#e0e0e0] rounded-md transition-all flex items-center justify-center gap-2"
            >
              <FiTrash2 size={13} />
              RESET SYSTEM
            </button>
            <p className="text-[9px] text-[#999] text-center">
              Clears all logs and resets the pipeline to initial state. Use before running a new audit cycle.
            </p>
          </div>
        </div>
      </div>
