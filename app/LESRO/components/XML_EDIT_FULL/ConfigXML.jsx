...
const ConfigXML = ({ externalSearchSKU }) => {
    ...
    useEffect(() => {
        const fetchXML = async () => {
            try {
                ...
                // ✅ CORRECCIÓN: Filtro por company_name 'LESRO' añadido
                ...
                if (data?.xml_raw) {
                    ...
                    const codes = [...doc.getElementsByTagName("Product")]
                        .map(p => p.getElementsByTagName("Code")[0]?.textContent)
                        .filter(Boolean);
                    setProducts(codes);
                }
            } catch (err) { 
                console.error("Error fetching LESRO XML:", err); 
            } finally { 
                setLoadingXML(false); 
            }
        };
    ...
    // Escuchar cambios del CSV (Búsqueda externa)
    useEffect(() => {
    ...
    const handleSearch = (sku, isAuto = false) => {
        ...
        setSelectedConfigs(prev => {
            ...
            const productNode = [...xmlDoc.getElementsByTagName("Product")]
                .find(p => p.getElementsByTagName("Code")[0]?.textContent === sku);
    ...
    if (loadingXML) return (
        <div className="p-10 flex items-center gap-3 bg-white h-full">
            <div className="w-4 h-4 border-2 border-[#464775]/20 border-t-[#464775] rounded-full animate-spin"></div>
            <span className="text-[10px] font-bold text-[#464775] uppercase tracking-widest">Sincronizando PIM LESRO...</span>
        </div>
    );
            ...
            <div className="p-4 bg-white border-b shrink-0" ref={dropdownRef}>
                <div className="relative">
                    <div className="flex items-center bg-[#F3F2F1] rounded px-3 border-b-2 border-transparent focus-within:border-[#464775] transition-all">
                        ...
                        <input 
                            type="text" 
                            placeholder="AÑADIR SKU DE LESRO..." 
                            className="w-full p-2.5 bg-transparent outline-none text-[11px] font-bold uppercase"
                            value={selectedSKU} 
                        ...
                        <div className="absolute w-full mt-1 bg-white shadow-2xl rounded border z-50 max-h-60 overflow-auto border-[#EDEBE9]">
                            {products.filter(p => p.includes(selectedSKU)).slice(0, 8).map(p => (
                                <div 
                                    ...
                                    className="p-3 hover:bg-[#F3F2F1] cursor-pointer text-[10px] font-black border-b last:border-none text-[#464775]"
                                >
                                    {p}
                                </div>
                            ))}
            ...
            <div className="flex-grow overflow-y-auto p-3 space-y-3">
                {selectedConfigs.length > 0 ? (
                        ...
                        >
                            ...
                            >
                                ...
                                <button 
                                    ...
                                    className="p-1 hover:bg-black/10 rounded text-inherit"
                                >
                                    <FiX size={12} />
                                </button>
                            </div>
                    ...
                    <div className="h-full flex flex-col items-center justify-center opacity-30 text-center p-10">
                        ...
                        <p className="text-[9px] mt-2 font-medium">No hay discrepancias seleccionadas de Lesro.</p>
                    </div>
                )}
            </div>
        </div>
