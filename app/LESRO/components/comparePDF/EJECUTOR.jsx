...
import { createPortal } from 'react-dom'; // Agregado para el popup
import { 
  ...
  FiMaximize2 // Icono para el botón de expansión
} from 'react-icons/fi';
...
const SVXUnifiedPlatform = () => {
  ...
  // --- COMPONENTE DE CONTENIDO (ABSTRACCIÓN PARA REUTILIZAR EN POPUP) ---
  ...
  return (
    <div className="h-[88vh] bg-[#FDFDFD] p-6 font-sans text-[#242424] max-w-[1600px] mx-auto space-y-4 relative overflow-hidden flex flex-col">
      ...
      <div className="grid grid-cols-12 gap-6 flex-grow min-h-0">
        ...
        <div className="col-span-9 flex flex-col gap-4 h-full min-h-0">
          ...
          <main className="flex-1 bg-white border border-[#EDEBE9] rounded-lg shadow-sm flex flex-col min-h-0 overflow-hidden relative group">
            {/* BOTÓN DE MAXIMIZAR AGREGADO */}
            <button 
              onClick={() => setIsMaximized(true)}
              className="absolute top-3 right-3 z-30 p-2 bg-white/80 hover:bg-[#464775] hover:text-white border border-[#EDEBE9] rounded-md shadow-sm transition-all opacity-0 group-hover:opacity-100 flex items-center gap-2 text-[10px] font-bold"
            >
      ...
      {/* PORTAL PARA EL POPUP (90% VIEWPORT + BLUR) */}
          ...
          >
            <div className="flex-shrink-0 bg-[#464775] p-4 flex justify-between items-center text-white">
              <div className="flex items-center gap-3">
                <Terminal size={18} />
                <span className="text-sm font-black uppercase tracking-widest">Enhanced Data Inspection - {activeTab.replace('_', ' ')}</span>
              </div>
              <button 
