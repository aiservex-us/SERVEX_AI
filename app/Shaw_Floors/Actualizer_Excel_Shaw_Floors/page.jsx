...
import XMLResults_Shaw_Floors from './components/XML_Results_Shaw_Floors.jsx';
import CETComparator from './components/CET_Comparator.jsx';
import CSVFinalResults_Shaw_Floors from './components/CSV_Final_Results_Shaw_Floors.jsx';
...
export default function ExcelActualizer() {
  ...
  useEffect(() => {
    const checkDelegation = async () => {
      try {
        ...
        if (!user) {
          window.location.href = '/Shaw Floors';
          return;
        }
        
        const apiURL = process.env.NEXT_PUBLIC_API_URL || 'https://servex-ai-back.onrender.com';
        const res = await fetch(`${apiURL}/api/v1/module_delegation/Shaw Floors`);
  ...
  const renderContent = () => {
    switch (active) {
      ...
      case 'incert_delete': return <IncertData moduleName="Shaw Floors" />;
      case 'xml_results': return <XMLResults_Shaw_Floors />;
      case 'cet_comparator': return <CETComparator />;
      case 'csv_final': return <CSVFinalResults_Shaw_Floors />;
  ...
  return (
    <div className="h-[97vh] w-[99%] bg-[#fff] font-sans flex items-center justify-center relative">
      <CriticalExcelModal xmlRoute="/Shaw_Floors/Actualizer_XML_Shaw_Floors" />
            ...
            <div className="px-8 py-6 flex gap-4">
              ...
              <div>
                ...
                <p className="text-[13px] text-[#616161] leading-relaxed">
                  You are about to leave the Shaw Floors management area. Any temporary changes in this view will be closed.
                </p>
              </div>
            </div>
