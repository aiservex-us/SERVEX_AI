...
const EJECUTOR_PLAY = ({ 
  ...
  const ejecutarSegundoProceso = async () => {
    ...
    try {
      // FORZADO A Shaw Floors: Eliminación de lógica condicional para asegurar persistencia única
      const targetCompany = 'Shaw Floors';
      ...
      // RUTA ÚNICA: Gateway exclusivo para el segmento Shaw Floors
      const endpointUrl = `${baseUrl}/wbs/api/v1/pipeline/compare-only-Shaw Floors`;
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.detail || 'Falla en la respuesta del motor de comparación Shaw Floors');
      }
  
      const result = await response.json();
      console.log('[✓] SERVEX_AI Engine Response (Shaw Floors):', result);
    ...
    <div className="flex flex-col gap-2 sm:gap-3 lg:gap-4 font-sans antialiased text-[#242424]">
        ...
        <div className="fixed inset-0 z-[1001] flex items-center justify-center bg-white/20 backdrop-blur-md animate-in fade-in duration-300 p-4 sm:p-6">
          <div className="bg-white border border-gray-200 shadow-2xl rounded-lg sm:rounded-2xl p-4 sm:p-6 max-w-sm w-full text-center space-y-3 sm:space-y-4 transform animate-in zoom-in-95 duration-200">
            ...
            <div className="space-y-1">
              ...
              <p className="text-[10px] sm:text-[11px] text-gray-500 font-medium">Catalog (Shaw Floors): {currentDate}</p>
            </div>
            ...
            <div className="flex items-center justify-center gap-2 text-[10px] font-bold text-[#5b5fc7]">
              ...
              <span className="uppercase tracking-widest">Synchronizing with Shaw Floors...</span>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[1001] flex items-center justify-center bg-white/20 backdrop-blur-md animate-in fade-in duration-300 p-4 sm:p-6">
          <div className="bg-white border border-gray-200 shadow-2xl rounded-lg sm:rounded-2xl p-4 sm:p-6 max-w-sm w-full text-center space-y-3 sm:space-y-4 transform animate-in zoom-in-95 duration-200">
            ...
            <div className="space-y-1">
              ...
              <p className="text-[10px] sm:text-[11px] text-gray-500 font-medium">Module (Shaw Floors): {currentDate}</p>
            </div>

            <div className="bg-amber-50 border border-amber-100 p-2 sm:p-3 rounded-lg sm:rounded-xl flex items-start gap-2 sm:gap-3 text-left">
              <FiAlertCircle className="text-amber-600 shrink-0 mt-0.5" size={14} />
              <p className="text-[9px] sm:text-[10px] text-amber-800 leading-tight">
                <strong>WARNING:</strong> Storing new Shaw Floors catalog data. Please wait for the data upload to complete.
              </p>
            </div>
      ...
      <div className="bg-white border border-[#e0e0e0] rounded-lg p-3 sm:p-4 lg:p-5 shadow-sm flex flex-col h-full gap-3 sm:gap-4">
        <div className="flex items-center gap-2">
          ...
          <h4 className="text-[10px] sm:text-[11px] font-bold uppercase tracking-wider text-[#616161]">System Actions (Shaw Floors)</h4>
        </div>
          ...
          <div className="flex flex-col gap-1.5 sm:gap-2 pb-3 sm:pb-4 border-b border-[#f0f0f0]">
            ...
            <div className="px-1 py-1 sm:py-2">
              ...
              <p className="text-[8px] sm:text-[9px] text-[#616161] leading-relaxed">
                Uploads your Shaw Floors catalog data to the cloud database. Sanitizes, validates, and stores pricing information.
              </p>
            </div>
          </div>
