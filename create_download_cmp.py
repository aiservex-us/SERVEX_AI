...
const DownloadResultXML = ({ moduleName = 'MODULE_NAME' }) => {
...
for mod, rel_path in modules.items():
    cmp_dir = os.path.join(app_dir, rel_path, 'components/comparePDF/IncertData/components')
    os.makedirs(cmp_dir, exist_ok=True)
    filepath = os.path.join(cmp_dir, 'Download_result_XML.jsx')
    
    # Replace MODULE_NAME with the actual module
    mod_content = content.replace('MODULE_NAME', mod)
