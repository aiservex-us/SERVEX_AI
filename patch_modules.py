...
MODULES = ['WBS', 'WBT', 'WBA', 'WBO', 'WBD', 'LESRO']
...
def patch_ai_contact(path, mod):
    ...
    if f"handle{mod}ImportStep" in content:
        print(f"Skipping AI_contact for {mod}, already patched")
        return

    # Patch global message return to include event listener
...
def patch_incert_xml(path, mod):
    ...
    if "saveSingleStep" in content:
        print(f"Skipping incertXML.tsx for {mod}, already patched")
        return
    ...
    # Inject saveSingleStep
    handle_save_target = "// --- Lógica de Saneamiento y Guardado ---"
  ...
  // --- Lógica de Saneamiento y Guardado (Manual para el paso general) ---"""
    
    content = content.replace(handle_save_target, save_single_step_code)
