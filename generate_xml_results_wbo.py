...
# Update menuLateral
menu_path = os.path.join(components_dir, "menuLateral.jsx")
with open(menu_path, "r") as f:
    menu_content = f.read()

if "id: 'xml_results'" not in menu_content:
    target_str = "{ id: 'incert_delete', label: 'Import Base excel & XML', icon: UploadCloud, sub: 'Ingestion' },"
    new_item = "  { id: 'xml_results', label: 'XML Results', icon: FileSpreadsheet, sub: 'Data' },"
    menu_content = menu_content.replace(target_str, f"{target_str}\n{new_item}")
    
    with open(menu_path, "w") as f:
        f.write(menu_content)

# Update page.jsx
page_path = os.path.join(wbo_dir, "page.jsx")
with open(page_path, "r") as f:
    page_content = f.read()

if "import XMLResultsWBO" not in page_content:
    import_target = "import IncertData from './components/IncertDataExcel/Incert_data_excel.jsx';"
    new_import = "import XMLResultsWBO from './components/XML_Results_WBO.jsx';"
    page_content = page_content.replace(import_target, f"{import_target}\n{new_import}")
    
    route_target = "case 'incert_delete': return <IncertData moduleName=\"WBO\" />;"
    new_route = "case 'xml_results': return <XMLResultsWBO />;"
    page_content = page_content.replace(route_target, f"{route_target}\n      {new_route}")
    
    # Let AI chat remain visible on xml_results
    with open(page_path, "w") as f:
        f.write(page_content)

print("Created XMLResultsWBO component and wired it up successfully.")
