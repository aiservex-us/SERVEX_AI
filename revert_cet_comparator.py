...
for file_path in files:
    ...
    old_base_price_logic = """const basePrice = priceNode ? parseFloat(priceNode.getElementsByTagName('Value')[0]?.textContent || "0") : 0;"""
    
    content = re.sub(new_base_price_logic, old_base_price_logic, content)

    new_opt_price_logic = """const rawOpt = optPriceElem\?\.textContent \|\| "0";
              const optPrice = parseFloat\(rawOpt\.replace\(/,/g, '\.'\)\) \|\| 0;"""

    old_opt_price_logic = """const optPrice = optPriceElem ? parseFloat(optPriceElem.textContent || "0") : 0;"""

    content = re.sub(new_opt_price_logic, old_opt_price_logic, content)
