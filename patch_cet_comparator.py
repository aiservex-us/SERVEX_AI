...
for mod in modules:
    ...
    for d in os.listdir(mod_dir):
        if d.startswith('Actualizer_Excel_'):
            excel_dir = d
    if excel_dir:
        comp_path = os.path.join(mod_dir, excel_dir, 'components', 'CET_Comparator.jsx')
        if mod == 'LESRO':
            comp_path = os.path.join(mod_dir, 'components', 'CET_Comparator.jsx')
...
"""
            ...
            # Remove alert("Comparison completed and saved successfully!");
