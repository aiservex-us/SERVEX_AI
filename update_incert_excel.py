...
for mod, suffix in mod_suffixes.items():
    ...
    # Path to DeleteData
    if mod == "LESRO":
        delete_data_path = "../../../components/comparePDF/IncertData/components/delete_data"
    else:
        xml_folder = f"Actualizer_XML_{suffix}"
        delete_data_path = f"../../../../{xml_folder}/components/comparePDF/IncertData/components/delete_data"
        
    incert_data_content = f"""'use client';

import React, {{ useState }} from 'react';
import InsertXML from './incertXML_excel';
import DeleteData from '{delete_data_path}';
...
"""
    ...
    if os.path.exists(menu_path):
        ...
        pattern = r"\{ id: 'incert_delete', label: 'Import CET XML', icon: UploadCloud, sub: 'Ingestion' \}"
        new_item = f"{{ id: 'incert_delete', label: 'Import Base excel & XML', icon: UploadCloud, sub: 'Ingestion' }}"
        
        if "Import CET XML" in content:
            content = re.sub(pattern, new_item, content)
        else:
            # If for some reason it wasn't added by the previous script
            pattern_full = r"const menuItems = \[\s*\{ id: 'reporting', label: '.*?' Home', icon: LayoutDashboard, sub: 'Dashboard' \},\s*\{ id: 'converter', label: 'XML to CSV', icon: FileSpreadsheet, sub: 'Data Converter' \},\s*\];"
  ...
  {{ id: 'incert_delete', label: 'Import Base excel & XML', icon: UploadCloud, sub: 'Ingestion' }},
            ...
            content = re.sub(pattern_full, new_items, content, flags=re.DOTALL)
            
        with open(menu_path, "w") as f:
            f.write(content)
            
    print(f"Updated CET XML Ingestion for {mod}")

print("Done updating IncertDataExcel.")
