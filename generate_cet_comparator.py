...
export default function CETComparator() {
  ...
  const computeComparison = async () => {
    ...
    if (!activeRecord.xml_actualizer_raw) {
      toast.error("Baseline XML (xml_actualizer_raw) not found.");
      return;
    }
    if (!activeRecord.XM_CET_import) {
      toast.error("Modified XML (XM_CET_import) not found.");
      return;
    }

    setIsComputing(true);
    const computeToast = toast.loading("Analyzing XML DOMs and computing deltas...");
    ...
    try {
      ...
      toast.loading("Saving analysis to database...", { id: computeToast });
      ...
      toast.success("Comparison completed and saved successfully!", { id: computeToast });

    } catch (err) {
      console.error(err);
      toast.error(`Error during comparison: ${err.message}`, { id: computeToast });
    } finally {
      ...
      <div className="w-full max-w-[90vw] mx-auto">
        ...
        <div className="mb-6 rounded-lg p-10 border border-[#464775]/20 bg-gradient-to-tr from-white/90 via-white/80 to-[#464775]/5 backdrop-blur-md flex flex-col items-center justify-center text-center shadow-[0_2px_15px_rgba(70,71,117,0.05)] relative overflow-hidden">
           ...
           <button 
             ...
             className="absolute top-4 right-4 flex items-center gap-2 px-4 py-2 bg-[#464775] text-white rounded-md text-xs font-semibold hover:bg-[#34355a] transition-all disabled:opacity-50 z-20 shadow-sm"
           >
...
print("Created CET_Comparator.jsx successfully.")
