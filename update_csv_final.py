...
file_path = "/Users/glynne/Desktop/SERVEX_AI/app/WBT/Actualizer_Excel_Tables/components/CSV_Final_Results_WBT.jsx"

with open(file_path, "r") as f:
    content = f.read()

# 1. Change component name
content = content.replace("WBODataMatrix", "CSVFinalResultsWBT")
content = content.replace("export default WBODataMatrix;", "export default CSVFinalResultsWBT;")
...
start_marker = "const parser = new DOMParser();"
end_marker = "setOptionHeaders(dynamicOptionHeaders);"
# Also remove the whole `for (const p of productsXML)` loop
loop_end_marker = "setProducts(extracted);"

# Actually it's easier to regex replace the whole processXML body
...
new_process_xml_body = """const processXML = async () => {
    try {
      ...
      setError(err.message || "Error processing CSV Final information WBT.");
    } finally {
      setLoading(false);
    }
  };"""
...
new_export = """const exportToCSV = () => {
    ...
    link.href = url;
    link.setAttribute('download', `WBT_CSV_Final_${new Date().toISOString().slice(0,10)}.csv`);
...
# Also fix the text "Product Database - WBT" and stats
content = content.replace("Product Database - WBT", "CSV Final Database - WBT")
...
# Remove any remaining optionHeaders/baseHeaders mentions that broke
