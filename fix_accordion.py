...
for mod in modules:
    ...
    if os.path.exists(contact_path):
        ...
        # We need to replace the old UI block. It looks something like:
        # <div className="p-1.5"> ... {SLASH_COMMANDS.filter(...) ... </div>
        # And is inside <motion.div ...> ... </motion.div>
        
        # We can find the exact bounds using a regex that captures from `<div className="p-1.5">` 
        # up to `</motion.div>` that precedes the main textarea `motion.div`.
        
        pattern = re.compile(r'<div className="p-1\.5">\s*<div className="px-2 py-1 text-\[8px\] font-bold uppercase tracking-widest text-gray-400 border-b border-gray-50 mb-0\.5">\s*Audit Commands\s*</div>.*?</div>\s*</motion\.div>', re.DOTALL)
        ...
        if count > 0:
            print(f"Patched accordion UI for {mod}")
        else:
